...
if(nf)nf.addEventListener('submit',async e=>{
  ...
  try{
    const {error}=await c.rpc('subscribe_newsletter',{p_email:email,p_source:'website',p_consent_version:'2026.09'});
    if(error)throw error;
    msg.textContent='¡Listo! Tu suscripción quedó activa.';
    nf.reset();
  }catch(error){
    console.error('Newsletter:',error);
    msg.textContent='No pudimos guardar tu suscripción. Intenta nuevamente.';
  }
