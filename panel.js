...
async function loadEntrepreneurSnapshot() {
  ...
  const [planRows, sales, saleItems, products, clients, payments, creditMoves, orders, expenses, suppliers, supplierInvoices, supplierPayments, growthGoals, accountantRows] = await Promise.all([
    ...
    safe(panelCloud.from("supplier_payments").select("*").eq("business_id", businessId).order("payment_date", {ascending:false}).limit(5000)),
  ...
  return { planTier:String(planRows[0]?.plan_tier || panelBusiness.plan_tier || panelBusiness.plan || "free").toLowerCase(), sales, saleItems, products, clients, payments, creditMoves, orders, expenses, suppliers, supplierInvoices, supplierPayments, growthGoals, accountant:accountantRows[0]||null };
}
...
async function requireProData(){ const d=await loadEntrepreneurSnapshot(); if(String(d.planTier)!=="pro"){renderPlanSection(); return null;} return {d,m:buildEntrepreneurMetrics(d)}; }
